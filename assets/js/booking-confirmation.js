...
function addToGoogleCalendar(details) {
    const { startDate, endDate } = formatCalendarDateTime(details.date, details.time);
    
    const title = encodeURIComponent(`NHS Career Boost - ${details.service}`);
    const description = encodeURIComponent(
        `Your NHS Career Boost consultation for ${details.service}.\n\n` +
        `Contact: daniel@nhscareerboost.co.uk\n` +
        `Phone: 01604 908464\n\n` +
        `Please ensure you have all relevant documents ready for the session.`
    );
    const location = encodeURIComponent('Online (Details will be sent via email)');
...
function addToAppleCalendar(details) {
    ...
    const uid = `nhs-career-boost-${Date.now()}@nhscareerboost.co.uk`;
    
    // Create iCalendar format
    const icsContent = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//NHS Career Boost//Consultation Booking//EN',
        'CALSCALE:GREGORIAN',
        'METHOD:PUBLISH',
        'BEGIN:VEVENT',
        `UID:${uid}`,
        ...
        `DTSTART:${dtstart}`,
        `DTEND:${dtend}`,
        `SUMMARY:NHS Career Boost - ${details.service}`,
        `DESCRIPTION:Your NHS Career Boost consultation for ${details.service}.\\n\\nContact: daniel@nhscareerboost.co.uk\\nPhone: 01604 908464\\n\\nPlease ensure you have all relevant documents ready for the session.`,
        'LOCATION:Online (Details will be sent via email)',
        'STATUS:CONFIRMED',
        'SEQUENCE:0',
        'BEGIN:VALARM',
        'TRIGGER:-PT30M',
        'DESCRIPTION:Reminder: NHS Career Boost consultation in 30 minutes',
        'ACTION:DISPLAY',
        'END:VALARM',
        'END:VEVENT',
        'END:VCALENDAR'
    ...
    link.download = `nhs-career-boost-${details.service.toLowerCase().replace(/\s+/g, '-')}.ics`;
