/**
 * NHS Career Boost - Contact Form Handler
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Error from server
                const errors = data.errors?.map(e => e.message).join('<br>') || data.message;
    ...
    phoneInput.addEventListener('blur', function() {
        ...
        if (phone && phone.length > 0 && (phone.length < 10 || phone.length > 20)) {
            this.setCustomValidity('Phone number must be between 10 and 20 characters');
        } else {
            this.setCustomValidity('');
        }
    });
